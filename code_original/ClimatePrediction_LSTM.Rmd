...
history_df <- read.csv("df_history_pollution.csv")
df_results <- read.csv("df_pollution.csv")
df_results$sum_MSE <- df_results$MSE1+df_results$MSE2

df_results <- tidyr::gather(df_results,key = "key",value = "MSE", -c(lag,StepPred,lstm1_units,sum_MSE))
```
...
```{r Plotting, echo = FALSE}
best_model_index <- which.min(df_results$sum_MSE)
