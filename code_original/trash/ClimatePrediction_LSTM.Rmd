...
```{r,eval =F}
parameter_grid <- tidyr::expand_grid(lag_grid, lstm1_units_grid)
best_model_MSE <- Inf
df_results <- c()
df_history <- c()
...
for(i in 1:nrow(parameter_grid)){
  res <- LSTM_sinus_prediction(lag=as.integer(parameter_grid[i,1]), StepPred,
                               ...
                               scaledY = scaledY, y=y,ep=100,bs=30 )
  #save best model:
  ...
  write.csv(data.frame(df_results ),"df_pollution_results.csv", row.names = FALSE)
  
  #save results
  df_history  <- rbind(df_history, res$history)
  write.csv(data.frame(df_history ),"df_pollution_history_.csv", row.names = FALSE)
...
model_saved<-load_model_hdf5("model_pollution.h5")# load whole model
history_df <- read.csv("df_pollution_history.csv")
df_results <- read.csv("df_pollution_results.csv")
...
true_lwrEnd <- test_lwrEnd[(best_lag+1):length(test_lwrEnd)]
# True
