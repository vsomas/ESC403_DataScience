...
dataX_test <- array(dataX[c((40*24):nrow(dataX))], dim = c( length( c((40*24):nrow(dataX)) ), lag, 1))
dataX <- array(dataX[c(0:(40*24 -1)),], dim = c( 40*24 -1, lag, 1))
dataY_test<- array(dataY[c((40*24):length(dataY))], dim = c( length( c((40*24):length(dataY)) ), 1, 1))
dataY <- array(dataY[c(0:(40*24 -1))], dim = c( 40*24 -1, 1, 1))
...
model %>%
  layer_lstm(units = 32,input_shape = c(lag,1), return_sequences = F ) %>% #32, return_sequences = T
  #layer_dense(units = 1,activation = "relu")%>%
  #layer_dense(units = 1,activation = "relu")%>%
  #layer_lstm(units = 32, return_sequences = F ) %>% #32, return_sequences = T
  layer_dense(units = 1)
summary(model)
```
